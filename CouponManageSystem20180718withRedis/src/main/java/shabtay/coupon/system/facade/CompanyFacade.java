...
public class CompanyFacade implements CouponClientFacade {
	...
	public void createCoupon(Coupon newCoupon) throws CouponAlreadyExistInCompanyDBException, InterruptedException {
		...
		Company company = companyDBDAO.getCompany(this.companyId);
		Set<Coupon> coupons = companyDBDAO.getCompany(this.companyId).getCoupons();
...
//		couponDBDAO.removeCoupon(couponDBDAO.getCouponByTitle(coupon.getTitle()));
	 ...
	 */
	public Coupon getCoupon(long coupunId) throws InterruptedException, NameOrIdNotExistException {
		if(companyDBDAO.findCouponById(this.companyId, coupunId) == null)
			throw new NameOrIdNotExistException(ConstantList.NAME_OR_ID_NOT_EXIST);
		logger.debug("getCoupon() executed get coupon by Id");
		return companyDBDAO.findCouponById(this.companyId, coupunId);
	}
	 ...
	 * @throws NameOrIdNotExistException 
	 */
	public Coupon getCouponByName(String coupName) throws InterruptedException, NameOrIdNotExistException {
		if(companyDBDAO.findCouponByName(this.companyId, coupName) == null)
			throw new NameOrIdNotExistException(ConstantList.NAME_OR_ID_NOT_EXIST);
		logger.debug("getCouponByName() executed get coupon by Name");
		return companyDBDAO.findCouponByName(this.companyId, coupName);
	}
	...
	public Collection<Coupon> getAllCoupon() throws InterruptedException {
		logger.debug("getAllCoupon() executed " + companyDBDAO.getCoupons(this.companyId));
		return companyDBDAO.getCoupons(this.companyId); 
	}
